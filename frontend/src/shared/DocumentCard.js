...
const DocumentCard = ({ document }) => {
    const { id, title, subject,doc,  desc,featured, reviews } = document;
    ...
    return (
        <div className="document__card">
            <Card>
                ...
                <h5 className="document__title"><Link to={`/documents/${id}`}>{title}</Link></h5>
                <div className="card__bottom d-flex align-items-center justify-content-beetween mt-3">
                    
                    <button className="btn booking__btn"><Link to={`/documents/${id}`}>show more..</Link></button>
                </div>
            </CardBody>
            </Card>
